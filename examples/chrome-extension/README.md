# HFI Pay — Chrome Extension (Dev)

A minimal Chrome extension for testing HFI Pay locally.
...
3. Set **Portal API URL** → `http://localhost:3100` (HFI dev server)
4. Set **Contract address** → deployed `HfiPayAttestedV1` address
...
The extension calls the portal server to get a quote (signed order), then sends the deposit transaction directly to the contract via your local node.
